...
// Quick install notification tables
try {
    ...
    if ($conn->query($settings_sql)) {
        ...
        echo "❌ Error creating notification settings table: " . $conn->error . "\n";
    }
    ...
    if ($users_result && $users_result->num_rows > 0) {
        ...
        foreach ($types as $type) {
            ...
            if ($conn->query($insert_sql)) {
                echo "✅ Default settings inserted for users - $type\n";
            }
        }
    }
    ...
    if ($managers_result && $managers_result->num_rows > 0) {
        ...
        foreach ($types as $type) {
            ...
            if ($conn->query($insert_sql)) {
                echo "✅ Default settings inserted for managers - $type\n";
            }
        }
    }
    
    echo "\n🎉 Notification system installation completed!\n";
    echo "You can now use the notification features.\n";
